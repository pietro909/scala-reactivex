...
class SimpleStreamingSuite extends TestKit(ActorSystem("SimpleStreaming"))
  ...
  test("filter even values") {
    ...
    val filter: Flow[Int, Int, NotUsed] = SimpleStreaming.filterEvenValues
    val tenStrings: ints.Repr[Int] = ints.take(n).via(filter)
    val eventualStrings: Future[Seq[Int]] = tenStrings.runWith(Sink.seq)
    val s: Seq[Int] = eventualStrings.futureValue
